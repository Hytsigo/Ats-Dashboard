...
import { deleteCandidateService } from "@/features/candidates/services/candidates.service";

type Params = {
  organizationId: string;
};

export function useDeleteCandidateMutation({ organizationId }: Params) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: deleteCandidateService,
    onSuccess: async () => {
